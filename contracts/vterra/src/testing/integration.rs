...
use moneymarket::market::MigrateMsg as MarketMigrateMsg;
use moneymarket::overseer::MigrateMsg as OverseerMigrateMsg;
...
fn create_old_contracts() -> (TerraApp, Addr, Addr) {
    ...
    let oracle_code_id = store_old_oracle_contract_code(&mut app);
    let interest_model_code_id = store_old_interest_model_code(&mut app);
    let distribution_model_code_id = store_old_distribution_model_code(&mut app);
    let market_code_id = store_old_market_contract_code(&mut app);
    let overseer_code_id = store_old_overseer_contract_code(&mut app);
    ...
    (app, market_addr, overseer_addr)
}

fn create_new_contract(app: &mut TerraApp, market_addr: Addr, overseer_addr: Addr) -> Addr {
    let owner = Addr::unchecked(OWNER);
    ...
    let res: MarketConfigResponseOld = app
        ...
        .unwrap();
    let aterra_addr = res.aterra_contract;
    ...
    let msg = VterraInstantiateMsg {
        ...
        overseer_addr: overseer_addr.to_string(),
        aterra_contract: aterra_addr.to_string(),
        stable_denom: "uust".to_string(),
...
fn migrate_contracts(
    ...
    let res: VterraConfigResponse = app
        .wrap()
        .query_wasm_smart(vterra_addr.clone(), &VterraQueryMsg::Config {})
        .unwrap();
    let vterra_cw20_addr = res.vterra_contract;

    // migrate market contract
    let msg = MarketMigrateMsg {
        vterra_anchor_addr: vterra_addr.to_string(),
        vterra_cw20_addr: vterra_cw20_addr.to_string(),
    };
    app.migrate_contract(owner.clone(), market_addr.clone(), &msg, market_code_id)
        .unwrap_err();
    ...
    let msg = OverseerMigrateMsg {
        ...
        dyn_rate_min: Decimal256::from_ratio(1000000000000u64, 1000000000000000000u64),
        dyn_rate_max: Decimal256::from_ratio(1200000000000u64, 1000000000000000000u64),
        vterra_contract_addr: vterra_addr.to_string(),
    };
    app.migrate_contract(owner, overseer_addr.clone(), &msg, overseer_code_id)
        .unwrap_err();
}

fn proper_initialization() -> (TerraApp, Addr, Addr, Addr) {
    let (mut app, market_addr, overseer_addr) = create_old_contracts();
    let vterra_addr = create_new_contract(&mut app, market_addr.clone(), overseer_addr.clone());
    migrate_contracts(
        &mut app,
        vterra_addr.clone(),
        market_addr.clone(),
    ...
    (app, vterra_addr, market_addr, overseer_addr)
}
